...
export function lookupRezago(municipio, estado) {
  ...
  let estNorm = stripAccents(estado);
  if (estNorm === 'estado de mexico' || estNorm === 'edomex') estNorm = 'mexico';
  if (estNorm === 'cdmx' || estNorm === 'distrito federal') estNorm = 'ciudad de mexico';
  
  let munNorm = stripAccents(municipio);
  // Special specific patches for Nominatim's weird names
  if (munNorm === 'huixquilucan de degollado') munNorm = 'huixquilucan';
